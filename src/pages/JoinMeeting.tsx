          ...
          if (meeting.meetingType === "1-on-1") {
            if (meeting.invitedUsers[0] === user?.uid || isCreator) {
              if (meeting.meetingDate === moment().format("L")) {
                ...
                createToast({ title: "Lecture has ended.", type: "danger" });
                navigate(user ? "/" : "/login");
              } else if (moment(meeting.meetingDate).isAfter()) {
                createToast({
                  title: `Lecture is ongoing ${meeting.meetingDate}`,
                  type: "warning",
                });
                navigate(user ? "/" : "/login");
              }
            ...
            if (index !== -1 || isCreator) {
              if (meeting.meetingDate === moment().format("L")) {
                ...
                createToast({
                  title: `Lecture is on ${meeting.meetingDate}`,
                  type: "warning",
                });
              }
            } else {
              createToast({
                title: `You are not permitted to this lecture.`,
                type: "danger",
              });
              navigate(user ? "/" : "/login");
            }
